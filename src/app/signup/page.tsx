...
function InputField({
  ...
  return (
    <div className="space-y-1.5">
      <label htmlFor={id} className="block text-[12px] font-medium text-white/40 uppercase tracking-[0.12em]">
        {label}
      </label>
      <input
        id={id}
...
/* Free tier perks shown beside the form */
const PERKS = [
  "1 retainer client free forever",
      ...
      >
        ...
        >
          ...
          <div className="px-8 pt-8 pb-6 text-center border-b border-white/[0.06]">
            ...
            <h1 className="text-[22px] font-bold tracking-tight text-white mb-1">Create your account</h1>
            <p className="text-[13px] text-white/35">Free forever for your first client</p>

            {/* Perks — tiny trust builders right in the header */}
            <div className="flex flex-col gap-1.5 mt-4">
              {PERKS.map((perk) => (
                <div key={perk} className="flex items-center justify-center gap-2">
                  <div className="w-3.5 h-3.5 rounded-full bg-violet-500/15 border border-violet-500/25 flex items-center justify-center flex-shrink-0">
                    <Check className="w-2 h-2 text-violet-400" />
                  </div>
                  <span className="text-[11px] text-white/30">{perk}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="px-8 py-6 space-y-4">
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <InputField id="name" label="Full name" type="text" placeholder="Alex Chen" value={name} onChange={setName} />
              <InputField id="email" label="Email" type="email" placeholder="you@example.com" value={email} onChange={setEmail} />
              <InputField id="password" label="Password" type="password" placeholder="At least 6 characters" value={password} onChange={setPassword} minLength={6} />
            ...
            <p className="text-center text-[11px] text-white/18 pt-1">
              ...
              <Link href="/terms" className="text-white/30 hover:text-white/50 transition-colors underline underline-offset-2">Terms</Link>
              {" "}and{" "}
              <Link href="/privacy" className="text-white/30 hover:text-white/50 transition-colors underline underline-offset-2">Privacy Policy</Link>
            </p>
          </div>
